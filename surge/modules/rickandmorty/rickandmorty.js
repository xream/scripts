...
!(async () => {
  let count = $.getdata('rickandmorty_count') || 0
  if (!count) {
    ...
    count = $.lodash_get(body, 'info.count')
    $.log('ℹ️ count', count)
    if (count < 1) throw new Error('获取角色数量失败')
    $.setdata(count, 'rickandmorty_count')
  }
  $.log('ℹ️ count', count)
  let id = Math.floor(Math.random() * (parseInt(count, 10) + 1))
  $.log('ℹ️ id', id)
  const res = await http({
  ...
  const image = $.lodash_get(body, 'image')
  $.log('ℹ️ image', image)
  if (!image) throw new Error('获取角色图片失败')
  const name = $.lodash_get(body, 'name')
  const url = $.lodash_get(body, 'url')
  const origin = $.lodash_get(body, 'origin.name') || ''
  const info = {
    status: $.lodash_get(body, 'status') || '-',
    species: $.lodash_get(body, 'species') || '-',
    gender: $.lodash_get(body, 'gender') || '-',
  }
  await notify(
    name || 'Rick and Morty',
    origin,
  ...
  .catch(async e => {
    $.logErr(e)
    $.logErr($.toStr(e))
    await notify('Rick and Morty', '❌', `${$.lodash_get(e, 'message') || $.lodash_get(e, 'error') || e}`)
  })
  .finally(async () => {
    $.done(result)
  })
...
function capitalizeFirstLetter(str) {
  ...
  return firstChar + str.slice(1).toLocaleLowerCase()
}
// prettier-ignore
