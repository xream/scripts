...
!(async () => {
  if($.isRequest()){
    // 偷懒了 自己保证格式正确
    let domain = $request.url.split('?domain=')[1]
    let ip = []
    try {
      const res = await $.http.get(`http://119.28.28.28/d?dn=${domain}`)
      let ipSet = new Set()
      res.body.split(/\r?\n/g).map(i => {
        let j = i.split(':')
        j = j[1] || j[0] || ''
    ...
    const body = `#!name=${domain || '未提供域名'}
#!desc=数量: ${ip.length}
[General]
force-http-engine-hosts = %APPEND% ${ip.join(' ')} ${domain.replace(/,/g, ' ')}
[MITM]
hostname = %APPEND% ${ip.join(' ')} ${domain.replace(/,/g, ' ')}`
    result = {
      response: {
        status: 200,
        body,
