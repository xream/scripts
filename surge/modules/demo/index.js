const NAME = `demo-${new Date().toLocaleTimeString('zh')}`
const $ = new Env(NAME)
...
!(async () => {
  if ($.isResponse()) {
    ...
    // 修改响应的例子
    result = { body: JSON.stringify({ ...body, modifiedBy: NAME }, null, 2)}
  } else if($.isRequest()){
    $.log('$request', $.toStr($request))
    // 修改请求的例子
    let { headers } = $request
    // 看情况 有的app需要处理大小写问题
    headers['User-Agent'] = `modifiedBy: ${NAME}`
    headers['user-agent'] = `modifiedBy: ${NAME}`
    result = { headers }
    // 不发出请求 直接返回构造好的响应的例子
    // result = {
