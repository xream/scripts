...
!(async () => {
  if ($.isRequest()) {
    ...
    const cache = $.getjson(KEY_CACHE)
    if (!cache) throw new Error('❌ 请先打开 Apple Store App 订单详情抓包')
    $.log('ℹ️ 读取缓存:')
    $.log($.toStr(cache))
    const opts = {
      url: cache.url,
  ...
  .catch(async e => {
    $.logErr(e)
    await notify(`Apple Store 订单查询`, `❌`, msg)
    result = {}
  })
  .finally(async () => {
    $.log($.toStr(result))
