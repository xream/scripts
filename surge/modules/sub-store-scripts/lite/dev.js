...
const SUB_STORE_SCHEMA = {
  ...
  author: '@xream',
  updateTime: '2023-10-18 00:54:21',
  version: '1.0.3',
  params: {
    host: {
      name: '混淆',
...
function operator(proxies = []) {
  ...
  return proxies
    .filter(p => {
      // snell 服务端没开混淆时 客户端硬加没用 直接过滤掉
      if (_.get(p, 'type') === 'snell' && !_.get(p, 'obfs-opts.mode')) {
        return false
      }
      return true
    })
