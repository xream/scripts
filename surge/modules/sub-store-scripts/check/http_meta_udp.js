...
async function operator(proxies = [], targetPlatform, context) {
  ...
  const url = `${http_meta_api}/udp`
  const ntp = $arguments.http_meta_protocol ?? 'time.apple.com'
  ...
  if (cacheEnabled) {
    try {
      ...
      for (var i = 0; i < internalProxies.length; i++) {
        ...
        if (cached) {
          if (cached.udp) {
            proxies[proxy._proxies_index].name = `[UDP] ${proxies[proxy._proxies_index].name}`
          }
        } else {
          allCached = false
          break
  ...
  async function check(proxy) {
    ...
    try {
      ...
      const res = await http({
        ...
        body: JSON.stringify({
          ntp,
          port: http_meta_ports[index],
          timeout: parseFloat($arguments.timeout || 3000),
        }),
      })
  ...
  function getCacheId({ proxy = {}, url }) {
    return `http-meta:udp:${url}:${JSON.stringify(
